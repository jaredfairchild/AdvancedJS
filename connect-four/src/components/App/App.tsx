...
export default class App extends React.PureComponent<Props, State> {
  ...
  handleTileClick = (tileId: string) => {
    ...
    // Change player turn
    const newPlayerTurn = playerTurn === "blue" ? "green" : "blue";
    // Calculate game status
    const gameStatus = this.calculateGameStatus(newPlayerTurn, newChipsPositions);
    // Save new state
    this.setState({ chipsPositions: newChipsPositions, playerTurn: newPlayerTurn, gameStatus, bgPlaying: true });
    };
  ...
  renderBoard() {
    const { columns, rows } = this.props;
    const { chipsPositions, bgPlaying } = this.state;
    return (
      <div>
        <Board
          columns={columns}
